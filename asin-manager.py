...
st.caption("Filtra campañas y grupos, luego introduce lista de ASINs para activar/desactivar. CSV final listo para descargar.")
    ...
    if st.button("✅ Validar ASINs y generar vista previa"):
        if not st.session_state.filtrado_listo:
            ...
            if df_resultado.empty:
                ...
                # =========================
                # #anotación: CSV Amazon Bulk Changes con Product/Entity/Operation correcto
                df_csv = df_resultado.copy()
                df_csv['Product'] = 'Sponsored Products'
                df_csv['Entity'] = 'Product Ad'
                df_csv['Operation'] = 'Update'  # siempre Update
                ...
                df_csv['State'] = 'enabled' if accion == 'enable' else 'disabled'
